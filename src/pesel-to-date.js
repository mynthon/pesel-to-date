...
const FORMAT_STRING_ARRAY = 'string-array';
const FORMAT_INT_ARRAY = 'int-array';
...
function peselToDate (pesel, format) {
  ...
  const century = 1800 + Math.floor(validMonths.indexOf(month) / 12) * 100;
  const fullYear = century + parseInt(year);
  const realMonth = validMonths.indexOf(month) % 12 + 1;
  const realMonthString = realMonth < 10 ? '0' + realMonth : '' + realMonth;
...
export { peselToDate, FORMAT_STRING, FORMAT_INT, FORMAT_DATE, FORMAT_STRING_ARRAY, FORMAT_INT_ARRAY };
